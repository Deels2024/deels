(function () {
  ...
  function ensureWalletTools() {
    ...
    nav.innerHTML = items.map(function (item) {
      ...
      return '<a class="' + (active ? 'active' : '') + '" href="' + item[2] + '"><strong>' + item[1] + '</strong><small>' + item[3] + '</small></a>';
    }).join('');
  ...
  function ensureWalletGuidance() {
    ...
    guide.innerHTML = '<strong>Условия вывода</strong><ul><li>Минимальная сумма — 500 ₽.</li><li>Комиссия проекта — 20%; к выплате поступает 80% запрошенной суммы.</li><li>Одновременно может обрабатываться только одна заявка.</li><li>Повторный вывод доступен не чаще одного раза в 30 дней.</li><li>Фактическая доступность и реквизиты повторно проверяются сервером.</li></ul>';
    ...
    function validateAmount() {
      ...
      if (!value) { status.textContent = ''; button.removeAttribute('aria-disabled'); return true; }
      if (value < 500) {
    ...
    button.addEventListener('click', function (event) {
      if (!validateAmount()) { event.preventDefault(); event.stopImmediatePropagation(); amount.focus(); }
    }, true);
  }
  ...
  function ensureGuestContestPrompt() {
    ...
    prompt.innerHTML = '<div><strong>Войти, чтобы увидеть доступные действия</strong><span>После входа Deels покажет, можно ли участвовать, принять вызов или выполнить другое доступное действие.</span></div><a href="/login?next=' + next + '">Войти</a>';
  ...
  function enhanceContestState() {
    ...
    var labels = { invited: 'Вас пригласили', participating: 'Вы участвуете', available: 'Можно участвовать', finished: 'Завершено', closed: 'Набор закрыт' };
  ...
  function ensureCampaignManagementState() {
    if (window.location.pathname.indexOf('/dashboard/my_campaigns/edit_campaign/') !== 0) return;
    var page = document.getElementById('page-wrapper');
    if (!page || page.dataset.deelsStateEnhanced === '1') return;
    page.dataset.deelsStateEnhanced = '1';
    if (!page.querySelector('.categories-lists')) {
      var form = page.querySelector('form');
      if (form) {
        var empty = document.createElement('div');
        empty.className = 'deels-campaign-empty-state';
        empty.innerHTML = '<strong>Пока здесь ничего нет</strong><span>Добавьте первый элемент через форму выше. После сохранения он появится в списке и будет доступен для редактирования.</span>';
        form.parentNode.insertBefore(empty, form.nextSibling);
      }
    }
    var alerts = page.querySelectorAll('.alert, .help-block');
    Array.prototype.forEach.call(alerts, function (alert) { alert.classList.add('deels-campaign-feedback'); });
  }

  function ensureMessagingStates() {
    if (window.location.pathname !== '/dashboard/messages') return;
    var chat = document.querySelector('.chat');
    if (!chat) return;
    if (!chat.querySelector('.deels-chat-network-state')) {
      var state = document.createElement('div');
      state.className = 'deels-chat-network-state';
      state.setAttribute('role', 'status');
      state.setAttribute('aria-live', 'polite');
      chat.insertBefore(state, chat.firstChild);
      function syncNetworkState() {
        if (navigator.onLine) {
          state.className = 'deels-chat-network-state is-online';
          state.textContent = 'Соединение восстановлено';
          window.setTimeout(function () { if (navigator.onLine) state.textContent = ''; }, 1800);
        } else {
          state.className = 'deels-chat-network-state is-offline';
          state.textContent = 'Нет соединения. Отправка сообщений возобновится после подключения к интернету.';
        }
      }
      window.addEventListener('online', syncNetworkState);
      window.addEventListener('offline', syncNetworkState);
      if (!navigator.onLine) syncNetworkState();
    }
    var list = chat.querySelector('.chat-list');
    if (list && !list.querySelector('.chat-item, .chat-list__item') && !list.querySelector('.deels-chat-empty')) {
      var emptyList = document.createElement('div');
      emptyList.className = 'deels-chat-empty';
      emptyList.innerHTML = '<strong>Диалогов пока нет</strong><span>Откройте профиль пользователя и нажмите «Написать», чтобы начать общение.</span>';
      list.appendChild(emptyList);
    }
    var body = chat.querySelector('.messages_body');
    if (body && !body.querySelector('.chat-message, .message') && !body.querySelector('.deels-chat-empty')) {
      var emptyBody = document.createElement('div');
      emptyBody.className = 'deels-chat-empty deels-chat-empty--messages';
      emptyBody.innerHTML = '<strong>Начните диалог</strong><span>Первое сообщение появится здесь.</span>';
  ...
  function observeFunctionalStates() {
    ...
    var observer = new MutationObserver(function () {
      ensureWalletTools(); ensureWalletGuidance(); ensureDepositGuidance(); ensureGuestContestPrompt(); enhanceContestState(); ensureCampaignManagementState(); ensureMessagingStates();
    });
    observer.observe(document.body, { childList: true, subtree: true });
  }

  ready(function () {
    ensureWalletTools(); ensureWalletGuidance(); ensureDepositGuidance(); ensureGuestContestPrompt(); enhanceContestState(); ensureCampaignManagementState(); ensureMessagingStates(); observeFunctionalStates();
  });
})();