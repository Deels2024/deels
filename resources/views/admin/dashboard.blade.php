...
@section('content')
    ...
    @if(!Auth::user()->is_admin() && $pending_campaigns->count()===0 && $last_payments->count()===0)

        <div class="account__subtitle mt-4 mb-1" style="text-align: left">

        </div>

        <div class="profile-statistics bg-dark">
            <h2 class="profile-statistics__title">{{Auth::user()->fullname}}, приветствуем Вас на DEELS!</h2>
            <div class="profile-statistics__content">
                <p>Скоро тут будут ваши копилки</p>
            </div>
            <div class="profile-statistics__row mt-4">
                <div class="account__btn btn btn_fill" onclick="window.location='/dashboard/my_campaigns/start_campaign'">Создать копилку</div>
            </div>
        </div>
    ...
    @elseif($pending_campaigns->count() > 0)
        <style>
            .campaign_image {
                width: 100%;
                height: 200px;
        ...
        <div class="account__banks flex align_start">
            @foreach($pending_campaigns as $nc)
                @php
                ...
                >
                    <div class="campaign_image {!! in_array($nc->status, [2,3]) ? 'blocked_image' : '' !!}" style="background-image: url('{{ $nc->feature_img_url()->feature_image }}')"></div>
                    <div class="bank__content">
                        ...
                        @endif
                        <div class="dashboard_buttons bank__text flex">
                            <div class="edit_campaign dashboard_button" data-id="{{$nc->id}}" style="display: flex;align-items: center">
                                <span class="actions-link" style="background-image: url(/dist/images/admin_icons/icon-edit.svg); margin-right: 10px"></span>
                                Редактировать
                            </div>
                            @if(in_array($nc->status, [2,3]))
                                <div class="delete_campaign dashboard_button" data-id="{{$nc->id}}" style="display: flex;align-items: center">
                                    <span class="actions-link" style="background-image: url(/dist/images/admin_icons/icon-del.svg); margin-right: 10px"></span>
                                    Удалить
                                </div>
                            @endif
                        </div>
...
@push('after_scripts')
    <script>
        $('.edit_campaign').on('click', function (e) {
            e.preventDefault();
            var route = '/dashboard/my_campaigns/edit_campaign'
            window.location=route+'/'+$(this).attr('data-id')
        });
        $('.delete_campaign').on('click', function (e) {
            e.preventDefault();
            e.stopPropagation();
            if (!confirm("<?php echo trans('app.are_you_sure'); ?>")) {
                return false;
            }
            var route = '/dashboard/my_campaigns/delete_campaign'
            window.location=route+'/'+$(this).attr('data-id')
        });
