    ...
    <main class="admin-main">
        <div class="account-main__head">
            <div class="account-main__head-title">
                <h1 class="account-main__title">Батлы
                </h1>
            </div>
        </div>

        <a href="{{route('battles.create')}}" class="d-flex ai-center gap-6 mb-4">
            ...
            <span class="fw-600 fz-5">Создать батл</span>
        </a>

        <div class="comments bg-dark">

            @if($battles && count($battles))
                <section class="challenge pb-8 pt-8">
                    <div class="challenge-grid" style="--challenge-grid: repeat(3, 1fr)">
                        @foreach($battles as $battle)
                            @include('battles.battle_item', ['route' => route('dashboard_battle_page', $battle->id), 'dashboard' => true, 'moderation' => true, 'show_ai_moderation' => true])
                        @endforeach
                    </div>
                    <div class="d-flex flex-column gap-8 ai-center jc-center pt-8">
                        @if($battles instanceof \Illuminate\Pagination\LengthAwarePaginator )
                            {{$battles->links()}}
                        @endif
                    </div>
                </section>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('body').on('click', '.comment_action', function (e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route('admin_battles.confirm') }}',
            ...
            $('.challenge_delete').on('click', function (e) {
                ...
                var battle_id = $(this).attr('data-id');

                $.ajax({
                    type: 'POST',
                    url: '{{ route('battles.remove') }}',
                    data: {battle_id: battle_id},
                    success: function (data) {
                        console.log(data);
                        if (data.success) {
                            $that.parents('.challenge-card').remove();
                            $('.alert-container').html('<div class="alert success"> <span class="closebtn">&times;</span>Батл удален!</div>')
                        } else {
                            alert('Невозможно удалить батл')
                        }
                    }
                });
            });
