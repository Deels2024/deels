...
@section('content')
    ...
    <main class="admin-main">
        ...
        <div class="d-flex mb-4">
            <a href="{{route('admin_battles_stories')}}" class="btn btn-sm mr-2" style="padding: 10px; font-size: 12px; line-height: 12px; height: auto; min-height: auto;">Все</a>
            <a href="{{route('admin_battles_stories')}}?type=frozen" class="btn btn-sm mr-2" style="padding: 10px; font-size: 12px; line-height: 12px; height: auto; min-height: auto;">На проверке</a>
            <a href="{{route('admin_battles_stories')}}?type=banned" class="btn btn-sm mr-2" style="padding: 10px; font-size: 12px; line-height: 12px; height: auto; min-height: auto;">Заблокированные</a>
        </div>

        <form action="{{route('admin_battles_stories')}}" type="GET" class="d-flex mb-4 align-items-center">
            <div class="">
                <input id="story_id" class="new__input" type="text" placeholder="№ сторис" name="story_id" value="{{$_GET['story_id'] ?? ''}}">
            </div>
            <div class="ml-3">
                <input id="story_id" class="new__input" type="text" placeholder="№ батла" name="battle_id" value="{{$_GET['battle_id'] ?? ''}}">
            </div>
                            ...
                            <div class="copystories-item__content">
                                ...
                                @if($story->battle)
                                    <div class="story-badge" href="">
                                        ...
                                        <span>Батл: <b>{{$story->battle->title}} (ID {{$story->battle->id}})</b>
                                            <span class="challenge_frozen" {!! $story->battle->frozen ? '' : 'style="display:none"' !!}>
                                                <br>(Заморожен)
                                            </span>
                                        </span>
                                    </div>
                                    <div class="d-flex align-items-center">
                                        <div class="like_btn like_btn_control btn btn-small" style="padding: 5px;" data-route="{{route('stories.like.web')}}" data-story="{{$story->id}}">👍️️ лайк <span>{{$story->votes_data['likes'] ?? 0}}</span></div>
                                        <div class="like_btn ml-2 mr-2 votes_count">({{$story->votes}})</div>
                                        <div class="dislike_btn like_btn_control btn btn-small" style="padding: 5px;" data-route="{{route('stories.dislike.web')}}" data-story="{{$story->id}}">👎️️️️ дизлайк  <span>{{$story->votes_data['dislikes'] ?? 0}}</span></div>
                                    </div>
                                @endif
    ...
    <script>
        $(document).ready(function () {
            $('body').on('click', '.comment_action', function (e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route('admin_battles_stories.confirm') }}',
        ...
        $('body').on('click', '.like_btn_control', function (e) {
            e.preventDefault();
            var like_btn = $(this);
                ...
                success: function (data) {
                    ...
                    if(data.success) {
                        if(data.votes) {
                            ...
                            $('.like_btn span').text(data.votes_data.likes)
                            $('.dislike_btn span').text(data.votes_data.dislikes)
                        } else {
                            $('.votes_count').text('('+data.votes+')');
                            $('.like_btn span').text(data.votes_data.likes)
                            $('.dislike_btn span').text(data.votes_data.dislikes)
