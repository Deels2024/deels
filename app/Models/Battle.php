...
class Battle extends Model
{
    ...
    {
        $now = Carbon::now();
        return $query->where('battles.active', 1)
            ->where('battles.declined', 0)
            ->whereNull('battles.blocked_at')
            ->where('battles.finished', false);
//            ->where('battles.start', '>=', $now);
    }
    ...
    {
        return $this->hasMany(Story::class, 'battle_id')
            ->withoutGlobalScopes()
    ...
    {
        return $this->hasOne(Story::class, 'battle_id')
            ->withoutGlobalScopes()
    ...
    public function getFile($path = false)
    {
        ...
        $filePath = $this->media->folder
            ...
            : 'uploads/battles/'.$this->media->slug_ext;
    ...
    public function getPathAttribute()
    {
        if ($this->media) {
            return $this->getTypeAttribute() === 'video'
                ? route('battles.get.video', $this->id)
                : $this->getFile();
        } else {
            return null;
        }
...
//    }
        ...
        if ($this->media && $this->getTypeAttribute() == 'video') {
            if ($this->media->thumbnail) {
                ...
                if (file_exists($videoUrl)) {
                    $file_path = 'uploads/stories/thumbs/battle_' . $this->id . '/';
    ...
    public function getParticipantsAttribute()
    {
        ...
        if ($this->user_id) {
            $participantIds->push((int) $this->user_id);
        }

        $storyParticipantIds = $this->stories()
            ->active()
            ->pluck('user_id')
            ->filter()
            ->map(static fn ($id): int => (int) $id);

        $participantIds = $participantIds->merge($storyParticipantIds);
    ...
    {
        if ($this->finished) {
            if ($this->completion_status === 'draw') {
                return 'Ничья';
            }
        ...
        if (!$this->started) {
            $requiredParticipants = (int) ($this->min_participants ?? 2);
            return $this->participants >= $requiredParticipants
                ? 'Запланирован'
                : 'Идет набор';
        }
