...
class Thread extends Eloquent
{
     ...
     * @var array
     */
    protected $fillable = ['subject', 'users'];
    ...
    {
        return $this->hasMany(Models::classname(\App\Models\Message::class), 'thread_id', 'id');
    }
    ...
    public function scopeForUser(Builder $query, $userId)
    {
        ...
        return $query->whereJsonContains('users', intval($userId))->orWhereJsonContains('users', (string)$userId);
...
//            ->select($threadsTable . '.*')->whereJsonContains('users', intval($userId));
    }
    ...
    public function scopeForUserWithNewMessages(Builder $query, $userId)
    {
        ...
        return $query->join($participantTable, $this->getQualifiedKeyName(), '=', $participantTable . '.thread_id')
            ...
            ->addSelect([
                $threadsTable . '.*',
                $this->getConnection()->raw("(SELECT COUNT(*) FROM {$messagesTable} WHERE {$messagesTable}.thread_id = {$threadsTable}.id) as messages_count")
            ]);
    }
    ...
    {
        if(in_array(0, $this->users) || !$this->users) {
            return 'DEELS';
        }
        $participantsTable = Models::table('participants');
    ...
    public function participantsAvatar($userId = null, $columns = ['name'])
    {
        if(in_array(0, $this->users) || !$this->users) {
            return '/default_avatars/robot.jpeg';
        }
        ...
        $users_ids = $participantNames->pluck('users.id')->toArray();
        if(!empty($users_ids)) {
    ...
    public function participantsId($userId = null, $columns = ['name'])
    {
        $participantsTable = Models::table('participants');
        $usersTable = Models::table('users');
        $userPrimaryKey = Models::user()->getKeyName();
