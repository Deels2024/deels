...
class SMSCHelper
{
    ...
    {
        $this->SMSC_LOGIN = env("SMSC_LOGIN", "login"); // логин клиента
        $this->SMSC_PASSWORD = env("SMSC_PASSWORD", "password");    // пароль
        $this->SMSC_POST = env("SMSC_POST", 0);                    // использовать метод POST
        $this->SMSC_HTTPS = env("SMSC_HTTPS", 0);                // использовать HTTPS протокол
        $this->SMSC_CHARSET = env("SMSC_CHARSET", "windows-1251");    // кодировка сообщения: utf-8, koi8-r или windows-1251 (по умолчанию)
        $this->SMSC_DEBUG = env("SMSC_DEBUG", 0);                // флаг отладки
        $this->SMTP_FROM = env("SMTP_FROM", "api@smsc.ru");     // e-mail адрес отправителя
...
// либо массив (<id>, -<код ошибки>) в случае ошибки
    ...
    function send_sms($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $soc = false, $sender = false, $query = "", $files = array())
    {
        ...
        $m = $this->_smsc_send_cmd("send", "cost=3&phones=" . urlencode($phones) . "&mes=" . urlencode($message) .
            "&soc=$soc&translit=$translit&id=$id" . ($format > 0 ? "&" . $formats[$format] : "") .
        ...
        try {
            if ($m[1] > 0)  {
                ...
                if($balance < 200) {
                    ...
                    $telegram->telegram_group_message('⚠️ Внимание! SMSC Баланс < 200 руб. ('.$balance.')');
                }
            }
